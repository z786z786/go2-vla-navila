...
from .zoh_closed_loop_probe7_floor1 import POLICY
...
def main():
    ...
    (root/'eval_plan.json').write_text(json.dumps({'stage':'DT3','training_root':str(args.training_root),
        ...
        'test_used':False,'training':False,'resource_probe_required':True,
        'promotion_condition':'first task success with independent audit and sampled free >=1024 MiB','selection':'validation closed-loop success, then wrong-stop/collision and arrival; no offline loss ranking',
    ...
    try:
        for task in tasks:
            ...
            if waiter.wait()['status']!='GPU_READY_LOCKED_RECHECKED':
                raise KeyboardInterrupt
            command=[str(DATA_ROOT/'envs/conda/navila-isaac/bin/python'),'-m','src.dual_target.zoh_closed_loop_probe7_floor1',
            ...
            child=None
            if len(results)==1:
                if evidence['episode_result']['status']!='success':
                    status('RESOURCE_PROBE_TASK_FAILED_AWAITING_REVIEW',detail='first trial not successful; lower admission not promoted')
                    return
                samples=[json.loads(line) for line in (root/f'{task["run_id"]}_gpu.jsonl').read_text().splitlines()]
                receipt=dict(status='LOWER_ADMISSION_ACCEPTED_FOR_THIS_MATRIX',
                    task=task,minimum_sampled_free_mib=min(s['free_mib'] for s in samples),
                    peak_owned_mib=max(s['owned_used_mib'] for s in samples),
                    required_free_mib=7168,runtime_free_floor_mib=1024,
                    scope='this validation matrix only; not a general GPU guarantee')
                if receipt['minimum_sampled_free_mib']<1024:raise RuntimeError('probe resource evidence failed')
                (root/'resource_probe_acceptance.json').write_text(json.dumps(receipt,indent=2)+'\n')
