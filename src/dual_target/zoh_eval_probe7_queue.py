...
from .zoh_closed_loop_probe7 import POLICY
...
def main():
    ...
    (root/'eval_plan.json').write_text(json.dumps({'stage':'DT3','training_root':str(args.training_root),
        ...
        'promotion_condition':'first task success with independent audit and sampled free >=2048 MiB','selection':'validation closed-loop success, then wrong-stop/collision and arrival; no offline loss ranking',
    ...
    try:
        for task in tasks:
            ...
            if waiter.wait()['status']!='GPU_READY_LOCKED_RECHECKED':
                raise KeyboardInterrupt
            command=[str(DATA_ROOT/'envs/conda/navila-isaac/bin/python'),'-m','src.dual_target.zoh_closed_loop_probe7',
            ...
            if len(results)==1:
                ...
                receipt=dict(status='LOWER_ADMISSION_ACCEPTED_FOR_THIS_MATRIX',
                    ...
                    required_free_mib=7168,runtime_free_floor_mib=2048,
                    scope='this validation matrix only; not a general GPU guarantee')
                if receipt['minimum_sampled_free_mib']<2048:raise RuntimeError('probe resource evidence failed')
