...
from .zoh_scheduler_core import baseline_samples
...
def main():
    ...
    try:
        ...
        command = [str(DATA_ROOT/'envs/conda/smolvla/bin/python'),'-m','src.dual_target.zoh_train_scheduler5k',
        ...
        if result['status'] != 'ZOH_SCHEDULER_5000_COMPLETE_AWAITING_REVIEW':
            raise RuntimeError('unexpected training result')
        status('ZOH_SCHEDULER_5000_COMPLETE_AWAITING_REVIEW',result=result)
