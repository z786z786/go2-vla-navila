"""Fresh dataset loop: six complete real stopped holds before termination."""
from __future__ import annotations
import argparse
import math
from .zoh_control import ZohClock, ZohSpec, probe_command
from .zoh_dataset import TerminalHolds, low_stop
from .scoring import ScoreDecision
...
def run_live_dataset(
    ...
    *,
    ...
    if mode != "expert":
        raise ValueError("this collector accepts expert only")
    terminal_holds = TerminalHolds()
    ...
    segment_log = (episode_root / "high_level_segments.jsonl").open("x", buffering=1)
    ...
    try:
        ...
        for step_index in range(args.max_env_steps):
            ...
            proposal = None
            if fresh:
                segment_start_time = adapter.sim_time_s
                segment_start_index = step_index
                if mode == "policy":
            ...
            # The inherited scorer latches success. Preserve failure precedence
            # throughout the genuine terminal collection extension.
            for failed, status in ((any(hits), ScoreStatus.FAILED_COLLISION),
                                   (fallen, ScoreStatus.FAILED_FALLEN),
                                   (wrong_stop, ScoreStatus.FAILED_WRONG_TARGET_STOP),
                                   (evaluator_stop, ScoreStatus.FAILED_EXTERNAL_STOP)):
                if failed:
                    decision = ScoreDecision(status, 0., "terminal collection failure")
                    break
            valid_stop = low_stop(raw_command, held_command, body_before, correct, asdict(thresholds)) and low_stop(
                raw_command, held_command, body, correct, asdict(thresholds))
            tail_ready = terminal_holds.observe(step_index, valid_stop)
            extension_lost = first_success_step is not None and not valid_stop
            if decision.status is ScoreStatus.SUCCESS and first_success_step is None:
            ...
            complete = (step_index + 1) % spec.hold_control_steps == 0
            failure = decision.terminal and decision.status is not ScoreStatus.SUCCESS
            if complete or failure or extension_lost or environment_done or step_index + 1 == args.max_env_steps:
                segment_log.write(json.dumps({
                    "high_level_index": step_index // spec.hold_control_steps,
                    "low_level_steps": step_index - segment_start_index + 1,
                    "actual_duration_s": adapter.sim_time_s - segment_start_time,
                    "complete": complete,
                }, allow_nan=False) + "\n")
            if extension_lost:
                raise Dt1RunnerError("stop conditions lost during real terminal extension; retain failed attempt")
            if failure or (decision.status is ScoreStatus.SUCCESS and tail_ready):
        ...
        result["first_success_low_level_index"] = first_success_step
        result["terminal_complete_zero_holds"] = terminal_holds.count
