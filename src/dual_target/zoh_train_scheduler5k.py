"""Fresh 5000-step official-scheduler ablation; all optimization variables otherwise fixed."""
import argparse
import gc
import json
...
from .smolvla_probe_client import DEFAULT_BASE_MODEL,DEFAULT_BASE_CONFIG_SHA256,DEFAULT_BASE_MODEL_SHA256,_build_new_3d_config
from .zoh_train import (FrameCache,emit,save_json,load_training_policy,checked_parameters,processors,profile,
    seed_all,optimizer_for,checkpoint,reload_processors,restore_rng)
from .zoh_scheduler_core import BASELINE,baseline_samples,assert_scope_matches,baseline_first_loss,make_scheduler,scheduler_config
...
def main():
    ...
    while free < 7168:
        emit(out,'waiting_gpu_before_model_allocation',free_mib=free,required_free_mib=7168)
        time.sleep(30)
        free=probe_gpu().free_mib
    ...
    save_json(out/'training_config.json', {
        ...
        'scheduler_reason':'User-authorized single-variable comparison against constant-LR 5000; same base, data, sample order, micro4/accum4, loss and frozen scope',
        ...
        'gradient_clip':10.,'max_updates_this_run':5000,'total_DT3_budget':10250,'old_updates_consumed':5250,'remaining_after_this_run':0,
        'budget_note':'Separate user-authorized fresh 5000-update scheduler experiment; no automatic continuation',
    ...
    for step in range(1,5001):
        ...
        if step in (1,250,500,750,1000,2000,3000,4000,5000):
            ...
            if step == 1:
                ...
                if abs(total_loss-baseline_first_loss())>1e-4:
                    raise ValueError('first pre-update loss differs from baseline; ablation inputs/RNG changed')
    ...
    save_json(out/'result.json',{'stage':'DT3','status':'ZOH_SCHEDULER_5000_COMPLETE_AWAITING_REVIEW',
        'optimizer_updates':5000,'checkpoint':str(out/'checkpoint_005000'),
        'next':'Stop; await comparison review. No automatic rollout, test use or further training'})
