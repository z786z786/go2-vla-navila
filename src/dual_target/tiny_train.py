"""Bounded DT3 pilot, invoked only by the CPU resource supervisor.
...
from .tiny_train_core import approved_dataset, file_sha, flow_loss, resource_candidates, choose_microbatch
...
class FrameCache:
    def __init__(self, root, manifest, out):
        ...
        dataset = LeRobotDataset('local/dual_target_v1_tiny', root=root/'lerobot')
        if len(dataset) != n:
    ...
    def batch(self, indices):
        ...
        future = idx[:,None] + torch.arange(50)[None,:]
...
def profile(policy, pre, cache, out):
    ...
    for micro in resource_candidates():
        try:
            ...
            if batch['action_is_pad'].shape != (micro,50):
            ...
            result = {'micro_batch': micro, 'passed': free >= 3072,
...
def main():
    ...
    free = probe_gpu().free_mib
    if free < 8192:
        raise RuntimeError('less than 8 GiB free before model allocation; requeue required')
    # Cap our allocator to admission free minus 3 GiB; outside-driver allocations
    # remain protected by the separate 2 GiB nvidia-smi watchdog.
    total = torch.cuda.get_device_properties(0).total_memory
    fraction = min(.90,(free-3072)*1024**2/total)
    ...
    sampled = torch.multinomial(weights,250*16,replacement=True,generator=sampler_gen)
    ...
    save_json(out/'training_config.json', {
        ...
        'scheduler_reason':'250-update pilot: do not inherit 1000-step warmup / 30000-step decay',
        ...
        'gradient_clip':10.,'max_updates_this_run':250,'total_DT3_budget':5000,
    ...
    fixed_noise = torch.randn((1,50,32),device='cuda',generator=torch.Generator(device='cuda').manual_seed(91))
    def prediction(model):
        model.eval()
        with torch.inference_mode(),torch.autocast('cuda',dtype=torch.bfloat16):
            result = model.predict_action_chunk(fixed,noise=fixed_noise).float().cpu()
        model.train()
        if result.shape != (1,50,3) or not bool(torch.isfinite(result).all()):
            raise ValueError('fixed observation/noise prediction failed')
        return result
    before_prediction = prediction(policy)
    emit(out,'training_started',micro_batch=micro,effective_batch=16,max_updates=250)
    ...
    save_json(out/'result.json',{'stage':'DT3','status':'PILOT_250_COMPLETE_NOT_DT3_APPROVED',
        'optimizer_updates':250,'checkpoint':str(out/'checkpoint_000250'),
        'next':'fixed instruction/color diagnostics and tiny closed-loop review; no automatic budget expansion'})
    emit(out,'pilot_complete',optimizer_updates=250,dt3_approved=False)
