...
from .tiny_train_core import approved_dataset, file_sha
...
POLICY = GpuAdmissionPolicy('dt3_train_shared_profile_8g_v1',8192,allow_existing_compute=True)
...
def main():
    ...
    try:
        ...
        command = [str(DATA_ROOT/'envs/conda/smolvla/bin/python'),'-m','src.dual_target.tiny_train',
        ...
        if result['status'] != 'PILOT_250_COMPLETE_NOT_DT3_APPROVED':
            raise RuntimeError('unexpected training result')
        status('PILOT_250_COMPLETE_NOT_DT3_APPROVED',result=result)
