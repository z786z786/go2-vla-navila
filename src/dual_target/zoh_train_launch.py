...
def main():
    ...
    try:
        ...
        command = [str(DATA_ROOT/'envs/conda/smolvla/bin/python'),'-m','src.dual_target.zoh_train',
        ...
        if result['status'] != 'ZOH_1000_COMPLETE_AWAITING_REVIEW':
            raise RuntimeError('unexpected training result')
        status('ZOH_1000_COMPLETE_AWAITING_REVIEW',result=result)
