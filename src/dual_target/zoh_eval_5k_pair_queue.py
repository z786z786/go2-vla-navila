...
def main():
    ...
    for step in (5000,):
        ...
        tasks.extend(dict(slot=i,step=step,policy_seed=args.policy_seed,
            ...
            checkpoint_manifest_sha256=file_sha(manifest)) for i in range(16,20))
    (root/'eval_plan.json').write_text(json.dumps({'stage':'DT3','training_root':str(args.training_root),
        ...
        'test_used':False,'training':False,'resource_probe_required':False,
        'promotion_condition':'user-authorized next-stage paired validation; ordinary task failures count and continue','selection':'validation closed-loop success, then wrong-stop/collision and arrival; no offline loss ranking',
