...
RUN_ID='20260911_surface_ep1065_v1'
OUT=BASE/RUN_ID
...
def main():
    ...
    command=['/mnt/wxh/go2_short_vln/envs/conda/navila-isaac/bin/python',
        str(SOURCE/'scripts/validate_r2r_episode_load.py'),
        '--output',str(OUT),'--episode-id',EPISODE_ID,'--render',
    ...
    try:
        with (BASE/f'{RUN_ID}_gpu.jsonl').open('x',buffering=1) as samples, (BASE/f'{RUN_ID}.log').open('x') as log:
            ...
            summary=json.loads((OUT/'render_report.json').read_text())
            if proc.returncode!=0 or not summary.get('passed'):raise RuntimeError('surface diagnostic failed')
