...
POLICY=GpuAdmissionPolicy('zoh_autodl_no_state_expert10k_shared12g_v1',12288,allow_existing_compute=True)
...
def main():
    ...
    from scripts.zoh_compare_policy_rpc_no_state import PolicyClient
    p=r._live_parser()
    ...
    if not 0 <= args.slot < 24:
        raise ValueError('only train/validation slots 0..23 authorized; test forbidden')
    split,scene,task=grouped_tasks()[args.slot]
    assert split in ('train','validation')
    plan=json.loads(args.eval_plan.read_text())
    scope=plan.get('scope')
    if scope not in ('autodl-no-state-expert-10000-train-validation'):
        raise ValueError('explicit reviewed checkpoint comparison plan required')
    ...
    manifest.update(stage='DT3_NO_STATE',slot=args.slot,policy_seed=args.policy_seed,
                    policy_checkpoint=str(args.policy_checkpoint),comparison_scope=scope,
                    source_state_read=False,state_placeholder='constant_zero_vector_dim3')
    (root/'source_manifest.json').write_text(json.dumps(manifest,indent=2))
    def status(value,**extra):
        (root/'stage_status.json').write_text(json.dumps({'stage':'DT3_NO_STATE','status':value,
            'training':False,'evaluation_split':split,'scope':scope,'navigation_success_approved':False,
            'source_state_read':False,'state_placeholder':'constant_zero_vector_dim3',**extra},indent=2)+'\n')
