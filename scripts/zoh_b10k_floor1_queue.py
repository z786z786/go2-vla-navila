"""Single-exception B-10k train queue: 7 GiB admission and 1 GiB runtime floor."""
import json
import os
...
import scripts.zoh_b10k_train_common as common

ROOT=common.BASE/'b10k_train_rollout_0907_v2'
SEED=common.SEED


def main():
    # The reporting functions resolve their module-level ROOT at call time.
    # Override it only inside this isolated queue process; v1 evidence is untouched.
    common.ROOT=ROOT
    ...
    value=common.checkpoint();tasks=common.tasks_for(root,value)
    if len(tasks)!=16 or [x['slot'] for x in tasks]!=list(range(16)) or any(x['evaluation_split']!='train' for x in tasks):
        raise ValueError('B-10k floor1 train rollout matrix contract failed')
    ...
    common.write_json(root/'rollout_plan.json',{'stage':'DT3','scope':SCOPE,'training':False,
        ...
        'resource_exception':'user-authorized 7 GiB admission and 1 GiB runtime floor for this queue only',
        ...
        'admission_free_mib':7168,'runtime_free_floor_mib':1024})
    common.publish_training_evidence();common.publish([])
    child=None;results=[]
    def stop(*_):raise KeyboardInterrupt
    signal.signal(signal.SIGTERM,stop);signal.signal(signal.SIGINT,stop)
    def status(value,**extra):
        row={'stage':'DT3','status':value,'pid':os.getpid(),'completed':results,'planned':16,
            'validation_used':False,'test_used':False,'resource_exception':'7g_admission_1g_floor',**extra}
        common.write_json(root/'queue_status.json',row)
        common.write_json(ROOT/'delivery/progress.json',{'status':value,'pid':os.getpid(),
    ...
    try:
        for task in tasks:
            ...
            command=[str(DATA_ROOT/'envs/conda/navila-isaac/bin/python'),'-m','scripts.zoh_b10k_floor1_runtime',
            ...
            with (root/f'{task["run_id"]}.log').open('x') as log:
                ...
                with (root/f'{task["run_id"]}_gpu.jsonl').open('x') as samples:
                    while child.poll() is None:
                        ...
                        if sample['free_mib']<1024:raise RuntimeError('runtime free GPU memory below authorized 1024 MiB')
            ...
            common.write_json(ep/'policy_trace_review.json',review)
            video=export_video(ep,task,evidence['episode_result'],ROOT/'delivery/videos')
            results.append({**task,**evidence['episode_result'],'review':review,**video});common.publish(results);child=None
        if len(results)!=16:raise ValueError('incomplete B-10k train matrix')
        common.publish(results,complete=True)
        common.write_json(ROOT/'delivery/artifacts_sha256.json',{str(p.relative_to(ROOT/'delivery')):common.sha(p)
