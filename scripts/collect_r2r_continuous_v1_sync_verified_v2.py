...
from src.dual_target.low_level import synchronized_command_observation
...
def warmup_stable(s):
    """Conservative zero-command gate; does not replace terrain validation."""
    values=s['position_w']+s['quaternion_wxyz']+s['linear_body']+s['angular_body']
    if not all(math.isfinite(x) for x in values):return False
    w,x,y,z=s['quaternion_wxyz']
    norm=w*w+x*x+y*y+z*z
    if abs(norm-1.)>.02:return False
    upright=1.-2.*(x*x+y*y)/norm
    return (upright>math.cos(math.radians(15))
        and math.sqrt(sum(v*v for v in s['linear_body']))<.15
        and math.sqrt(sum(v*v for v in s['angular_body']))<.30)
...
def run(args):
    ...
    try:
        ...
        # Override the configured root height, never mutate the task source pose.
        offset=float(args.spawn_height_offset)
        if not math.isfinite(offset) or not .2<=offset<=.5:
            raise ValueError('spawn height offset must be finite and in [0.2, 0.5] m')
        start=episode['start_position']
        cfg.scene.robot.init_state.pos=(start[0],start[1],start[2]+offset)
        save(output/'spawn_config.json',{'source_start_position':list(start),
            'root_position':list(cfg.scene.robot.init_state.pos),'offset_m':offset,
            'rotation_wxyz':list(cfg.scene.robot.init_state.rot),
        ...
        def capture_reset(ids):
            if len(ids):
                ...
                event={'state':snapshot(),'stage':reset_stage,'terminated':bool(base.reset_terminated[0]),
                    'truncated':bool(base.reset_time_outs[0]),'active_terms':manager.active_terms}
                if hasattr(manager,'get_term'):
                    event['triggered_terms']=[name for name in manager.active_terms if bool(manager.get_term(name)[0])]
                if reset_stage=='navigation':reset_evidence.append(event)
                else:
                    warmup_resets.append(event)
                    save(output/'warmup_reset_events.json',warmup_resets)
            return orig_reset(ids)
        base._reset_idx=capture_reset
        def step(cmd):
            nonlocal obs
            physics_contacts.clear()
            obs=synchronized_command_observation(obs,env.proprio_obs_buf,cmd)
            with torch.no_grad():
        ...
        metadata['command_sync_sha256']=sha(Path(__file__).resolve().parents[1]/'src/dual_target/low_level.py')
        metadata['command_sync']='rebuild flattened history after current command update'
        save(output/'manifest.json',metadata)
        # Preserve every failed attempt separately from navigation resets.
        # Retries are diagnostic, not evidence that a terrain/spawn defect is fixed.
        warmup_attempts=0
        while True:
            warmup_attempts += 1
            warmup_reset_start=len(warmup_resets)
            stable_steps=0
            for i in range(250):
                pre=snapshot();a,_,done,_=step([0.,0.,0.])
                reset_happened=len(warmup_resets)>warmup_reset_start
                post=warmup_resets[-1]['state'] if reset_happened else snapshot()
                stable_steps=stable_steps+1 if warmup_stable(post) and not reset_happened else 0
                emit(warm,{'attempt':warmup_attempts,'index':i,'pre':pre,'post':post,'command':[0.,0.,0.],'stable_steps':stable_steps})
                if bool(done[0]) or reset_happened:break
                if i>=99 and stable_steps>=50:break
            if not bool(done[0]) and not reset_happened and stable_steps>=50:break
            # Do not call env.reset() after a termination: this Isaac task can
            # deadlock while rebuilding the scene.  The original evidence is
            # sufficient to reject this start; the guard will cleanly stop it.
            raise RuntimeError(f'warmup rejected on attempt {warmup_attempts}; no unsafe reset retry')
        save(output/'warmup_result.json',{'attempts':warmup_attempts,'stable_steps':stable_steps,
            'prior_reset_events':len(warmup_resets),'gate':'15deg tilt, 0.15m/s linear, 0.30rad/s angular; 50 consecutive steps after minimum 100 steps'})
        reset_stage='navigation'
...
def main():
    ...
    p.add_argument('--spawn-height-offset',type=float,default=.32)
