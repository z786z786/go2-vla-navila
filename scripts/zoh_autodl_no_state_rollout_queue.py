"""Fixed no-state expert-10k checkpoint, all train then validation tasks on neu-3070."""
import json
import os
import signal
import subprocess
...
from scripts.zoh_autodl_no_state_rollout_runtime import POLICY
from scripts.zoh_compare_video import export_video

BASE = DATA_ROOT/'outputs/dual_target_v2'
ROOT = BASE/'autodl_no_state10k_train_eval_0909_v1'
CHECKPOINT = BASE/'zoh_no_state_expert10k_autodl_0909_v1/checkpoint_010000'
SCOPE = 'autodl-no-state-expert-10000-train-validation'
SEED = 20260906
...
def main():
    ...
    try:
        ...
        sizes = {'training_state.pt':799195784, 'pretrained_model/model.safetensors':1099927592,
            'pretrained_model/config.json':2113, 'pretrained_model/policy_preprocessor.json':1543,
        ...
        if not config['train_expert_only'] or not config['freeze_vision_encoder']:
            raise ValueError('checkpoint is not expert-only')
        ...
        for slot,(split,scene,task) in enumerate(grouped_tasks()[:24]):
            ...
            tasks.append(dict(condition='B_no_state_expert_10k_autodl', slot=slot, step=10000,
        ...
        for task in tasks:
            ...
            command=[str(DATA_ROOT/'envs/conda/navila-isaac/bin/python'),'-m','scripts.zoh_autodl_no_state_rollout_runtime',
            ...
            review=audit_policy_episode(ep,result,allow_state_placeholder=True);write(ep/'policy_trace_review.json',review)
