"""Policy pipe client for the no-3-D-state rollout ablation."""
...
from src.dual_target.contracts import IMAGE_KEY, STATE_KEY, validate_policy_input
...
class PolicyClient:
    def __init__(self, checkpoint, seed, output):
        ...
        env = dict(
            os.environ,
            PYTHONPATH="/home/wxh/go2_short_vln:/mnt/wxh/go2_short_vln/third_party/lerobot/src",
            HF_HOME="/mnt/wxh/go2_short_vln/cache/huggingface",
            HF_HUB_OFFLINE="1",
        ...
        self.child = subprocess.Popen(
            [
                ...
                "-m",
                "src.dual_target.zoh_policy_server_no_state",
                "--checkpoint",
                str(checkpoint),
                "--seed",
                str(seed),
    ...
    def action(self, rgb_path, state, task):
        if not self.queue:
            # Do not even transmit the live Go2 state to the worker.  Keep the
            # three-value field only as a protocol-compatible zero placeholder.
            payload = validate_policy_input(
                {IMAGE_KEY: str(rgb_path), STATE_KEY: [0.0, 0.0, 0.0], "task": task}
            )
            ...
            record = {
                ...
                "execute_steps": 1,
                "action_dt_s": 0.2,
                "wall_wait_s": time.monotonic() - start,
                "source_state_read": False,
                "state_placeholder": "constant_zero_vector_dim3",
            }
