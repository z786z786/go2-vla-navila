...
POLICY=GpuAdmissionPolicy('zoh_three_compare_shared9g_v1',9216,allow_existing_compute=True)
...
def main():
    ...
    if scope not in ('three-5000-checkpoint-comparison','two-5000-checkpoint-train-comparison'):
    ...
    def status(value,**extra):
        (root/'stage_status.json').write_text(json.dumps({'stage':'DT3','status':value,
            'training':False,'evaluation_split':split,'scope':scope,'navigation_success_approved':False,**extra},indent=2)+'\n')
