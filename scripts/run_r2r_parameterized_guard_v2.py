...
RUN_ID='20260911_spawn_ep1065_corrected_v4'
OUT=BASE/RUN_ID
SOURCE=Path('/home/wxh/go2_short_vln')
EPISODE_ID='1065'
SPAWN_HEIGHT_OFFSET=.32
...
def main():
    ...
    cache=BASE/(RUN_ID+'_cache')
    cache.mkdir(exist_ok=False)
    env['XDG_CACHE_HOME']=str(cache)
    env['__GL_SHADER_DISK_CACHE_PATH']=str(cache/'nvidia')
    command=['/mnt/wxh/go2_short_vln/envs/conda/navila-isaac/bin/python',
        str(SOURCE/'scripts/collect_r2r_continuous_v1_spawn_corrected_v2.py'),'--expert','continuous',
        '--output',str(OUT),'--episode-id',EPISODE_ID,'--seed','20260910','--max-seconds','120',
        '--load_run','2024-09-25_23-22-02','--headless','--enable_cameras','--spawn-height-offset',str(SPAWN_HEIGHT_OFFSET)]
    assert Path(command[1]).is_file(),command[1]
    print(json.dumps({'verified_command':command}),flush=True)
    ...
    try:
        with (BASE/f'{RUN_ID}_gpu.jsonl').open('x',buffering=1) as samples, (BASE/f'{RUN_ID}.log').open('x') as log:
            ...
            last_progress=time.monotonic(); last_signature=None
            while proc.poll() is None:
                ...
                if 'Failed to create the raytracing pipeline.' in (BASE/f'{RUN_ID}.log').read_text(errors='replace'):
                    raise RuntimeError('fatal raytracing pipeline initialization failure')
                if time.monotonic()-started>status['timeout_s']:raise RuntimeError('2400 second timeout')
                candidates=[OUT/'warmup.jsonl',OUT/'low_level.jsonl',OUT/'frames.jsonl']
                signature=tuple((p.stat().st_size,p.stat().st_mtime_ns) for p in candidates if p.exists())
                if signature!=last_signature:
                    last_signature=signature;last_progress=time.monotonic()
                if time.monotonic()-last_progress>120:
                    raise RuntimeError('no_progress: output unchanged for 120 seconds')
                time.sleep(1)
            status['exit_code']=proc.returncode
            if not (OUT/'summary.json').exists():
                raise RuntimeError(f'collector exited {proc.returncode} without summary; inspect collector log')
...
if __name__=='__main__':
    ...
    parser.add_argument('--spawn-height-offset',type=float,default=.32)
    args=parser.parse_args()
    if not args.run_id.replace('_','').isalnum():parser.error('invalid run ID')
    if not .2<=args.spawn_height_offset<=.5:parser.error('height must be in [0.2, 0.5]')
