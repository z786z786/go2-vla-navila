"""Durable all-8 B-10k validation rollout queue; independent test forbidden."""
import json
import os
import signal
import subprocess
...
from scripts.zoh_b10k_eval_common import ROOT,SEED,sha,write_json,checkpoint,tasks_for,publish,publish_training_evidence
...
def main():
    ...
    if len(tasks)!=8 or [x['slot'] for x in tasks]!=list(range(16,24)) or any(x['evaluation_split']!='validation' for x in tasks):
        raise ValueError('B-10k validation matrix contract failed')
    sources=[*(SOURCE_ROOT/'src/dual_target').glob('*.py'),*(SOURCE_ROOT/'scripts').glob('zoh_b10k_eval*.py'),
        *(SOURCE_ROOT/'scripts').glob('zoh_compare_policy*.py'),SOURCE_ROOT/'scripts/zoh_compare_video.py',
        SOURCE_ROOT/'scripts/zoh_full_support.py',SOURCE_ROOT/'scripts/zoh_b10k_train_common.py']
    write_json(root/'rollout_plan.json',{'stage':'DT3','scope':SCOPE,'training':False,
        'evaluation_split':'validation-only','validation_used':True,'test_used':False,
        'selection':'fixed B expert-only scheduler checkpoint 10000; all 8 validation tasks',
        'resource_exception':'user-authorized 7 GiB admission and 1 GiB floor carried over for identical B rollout stack',
    ...
    def status(value,**extra):
        row={'stage':'DT3','status':value,'pid':os.getpid(),'completed':results,'planned':8,
            'validation_used':True,'test_used':False,'resource_exception':'7g_admission_1g_floor',**extra}
        write_json(root/'queue_status.json',row)
        write_json(ROOT/'delivery/progress.json',{'status':value,'pid':os.getpid(),
            'completed':len(results),'planned':8,**{k:v for k,v in extra.items() if k!='detail'},
    ...
    try:
        for task in tasks:
            ...
            command=[str(DATA_ROOT/'envs/conda/navila-isaac/bin/python'),'-m','scripts.zoh_b10k_eval_runtime',
            ...
            review['scope']='B-10k validation-layout rollout only; no test or training feedback'
        ...
        if len(results)!=8:raise ValueError('incomplete B-10k validation matrix')
        ...
        status('B_10K_VALIDATION_ROLLOUT_COMPLETE_AWAITING_USER',report=str(ROOT/'delivery/README.md'))
