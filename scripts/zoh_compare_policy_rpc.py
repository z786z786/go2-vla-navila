...
class PolicyClient:
    def __init__(self,checkpoint,seed,output):
        self.queue=deque()
        self.index=0
        self.metadata={'checkpoint':str(checkpoint),'policy_seed':seed,'chunk_size':5,
                       'execute_steps':1,'action_dt_s':0.2,'policy_inputs':[IMAGE_KEY,STATE_KEY,'task']}
        ...
        self.child=subprocess.Popen([
            '/mnt/wxh/go2_short_vln/envs/conda/smolvla/bin/python','-m','scripts.zoh_compare_policy_server',
    ...
    def action(self,rgb_path,state,task):
        if not self.queue:
            ...
            record={'chunk_index':self.index,'input':payload,'raw_chunk':chunk,
                    'execute_steps':1,'action_dt_s':0.2,'wall_wait_s':time.monotonic()-start}
