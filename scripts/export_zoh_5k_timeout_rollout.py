"""Export a completed timeout validation rollout without altering source evidence."""
import argparse
import hashlib
import json
...
def main():
    ...
    p.add_argument('--color',choices=['red','blue'],required=True);p.add_argument('--number',type=int,required=True)
    ...
    assert '_c5000_' in ep.parents[1].name
    assert all(r['task']==f'Go to the {a.color} box and stop in front of it.' for r in pre)
    assert not any(any(r['collision_latched_substeps']) for r in post)
    ...
    vf=("pad=512:608:0:64:color=white,"
        f"drawtext=text='Go to {a.color.upper()} box and stop in front.':x=12:y=9:fontsize=21:fontcolor=black,"
        f"drawtext=text='Checkpoint 5000 | Validation {a.number:02d} | 1x simulation':x=12:y=38:fontsize=16:fontcolor=black,"
