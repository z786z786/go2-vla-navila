...
from src.dual_target.zoh_scheduler_core import baseline_samples
...
POLICY = GpuAdmissionPolicy('zoh_full_train_profile_14g_v1',14336,allow_existing_compute=True)
...
def main():
    ...
    baseline_samples(args.dataset.resolve())
    ...
    try:
        ...
        command = [str(DATA_ROOT/'envs/conda/smolvla/bin/python'),'-m','scripts.zoh_full_train',
                   '--dataset',str(args.dataset.resolve()),'--approval',str(args.approval.resolve()),'--output',str(out)]
        receipt = {'stage':'DT3','supervisor_pid':os.getpid(),'run_id':out.name,
            ...
            'max_runtime_s':28800,'dt2_approval_sha256':file_sha(args.approval),
            'source_sha256':{str(x.relative_to(SOURCE_ROOT)):file_sha(x)
                for pattern in ('src/dual_target/*.py','scripts/zoh_full*.py','tests/dual_target/test_dt3*.py')
        ...
        with (out/'training.log').open('x') as log:
            ...
            with (out/'gpu_samples.jsonl').open('x') as samples:
                while child.poll() is None:
                    ...
                    if time.monotonic()-start > 28800:
                        raise TimeoutError('DT3 pilot exceeded 8-hour owned runtime bound')
                    time.sleep(2)
            if child.returncode != 0:
                raise RuntimeError(f'trainer exited {child.returncode}; preserve failure and checkpoints')
        result = json.loads((out/'result.json').read_text())
        if result['status'] != 'ZOH_FULL_5000_COMPLETE_AWAITING_REVIEW':
            raise RuntimeError('unexpected training result')
        status('ZOH_FULL_5000_COMPLETE_AWAITING_REVIEW',result=result)
