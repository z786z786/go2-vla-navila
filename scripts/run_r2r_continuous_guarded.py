...
def main():
    ...
    command=['/mnt/wxh/go2_short_vln/envs/conda/navila-isaac/bin/python',
        ...
        '--output',str(OUT),'--episode-id','1','--seed','20260910','--max-seconds','120',
        '--load_run','2024-09-25_23-22-02','--headless','--enable_cameras']
    status=dict(command=command,min_free_mib=2048,timeout_s=900,status='preflight')
    ...
    try:
        with (BASE/'20260910_ep1_continuous_v1_gpu.jsonl').open('x',buffering=1) as samples, (BASE/'20260910_ep1_continuous_v1.log').open('x') as log:
            pre=sample_group(-1);samples.write(json.dumps(pre)+'\n')
            if pre['free_mib']<2048:raise RuntimeError('preflight free memory below 2 GiB')
            ...
            while proc.poll() is None:
                ...
                if time.monotonic()-started>900:raise RuntimeError('900 second timeout')
                time.sleep(1)
