...
def main():
    ...
    if not 0 <= args.slot < 4:
        raise ValueError('only train diagnostic slots 0..3 are authorized')
    split,scene,task=grouped_tasks()[args.slot]
    assert split=='train'
    plan=json.loads(args.eval_plan.read_text())
    if plan.get('evaluation_split')!='train' or plan.get('scope')!='seen-training-layout-fit-check':
        raise ValueError('explicit training diagnostic plan required')
    ...
    def status(value,**extra):
        (root/'stage_status.json').write_text(json.dumps({'stage':'DT3','status':value,
            'training':False,'evaluation_split':'train','scope':'seen-training-layout-fit-check','navigation_success_approved':False,**extra},indent=2)+'\n')
