...
def run(args):
    ...
    try:
        ...
        # Let the frozen locomotion policy settle its joint targets before
        # judging the route start. These steps are diagnostic-only and are
        # never exported as training transitions.
        for _ in range(50):
            _,_,done,_=step([0.,0.,0.])
            if bool(done[0]):
                raise RuntimeError('low-level settle phase terminated before warmup')
