"""One physical B-10k train-layout rollout; validation and test are forbidden."""
import json
...
POLICY=GpuAdmissionPolicy('zoh_b10k_train_shared9g_v1',9216,allow_existing_compute=True)
SCOPE='b-10000-train-rollout'
...
def main():
    ...
    if not 0<=args.slot<16:raise ValueError('B-10k rollout only permits train slots 0..15')
    split,scene,task=grouped_tasks()[args.slot]
    if split!='train':raise ValueError('validation/test task reached B-10k runtime')
    plan=json.loads(args.rollout_plan.read_text())
    if (plan.get('scope')!=SCOPE or plan.get('policy')!=POLICY.policy_id
            or plan.get('validation_used') is not False or plan.get('test_used') is not False):
        raise ValueError('explicit train-only B-10k plan required')
    ...
    if (len(match)!=1 or match[0]['slot']!=args.slot or match[0]['evaluation_split']!='train'
            or match[0]['policy_seed']!=args.policy_seed or match[0]['checkpoint']!=str(args.policy_checkpoint)):
        raise ValueError('runtime task differs from frozen B-10k plan')
    ...
    manifest.update(stage='DT3',slot=args.slot,policy_seed=args.policy_seed,
        policy_checkpoint=str(args.policy_checkpoint),comparison_scope=SCOPE,evaluation_split='train')
    (root/'source_manifest.json').write_text(json.dumps(manifest,indent=2))
    def status(value,**extra):
        (root/'stage_status.json').write_text(json.dumps({'stage':'DT3','status':value,'training':False,
            'evaluation_split':'train','validation_used':False,'test_used':False,'scope':SCOPE,
