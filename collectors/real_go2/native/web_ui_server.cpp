...
std::string MakeJsonStatus(const UiStatusSnapshot& status)
{
    ...
    oss << "{"
        ...
        << "\"vy\":" << status.velocityY << ","
        << "\"vz\":" << status.velocityZ << ","
        << "\"yaw_speed\":" << status.yawSpeed << ","
        << "\"error_code\":" << status.errorCode << ","
        << "\"mode\":" << status.mode << ","
        << "\"gait_type\":" << status.gaitType
        << "},"
        << "\"command\":{"
        << "\"vx\":" << status.commandVx << ","
        << "\"vy\":" << status.commandVy << ","
        ...
        << "},"
        << "\"d435i\":{"
        << "\"enabled\":" << BoolJson(status.d435iEnabled) << ","
        << "\"active\":" << BoolJson(status.d435iActive) << ","
        << "\"degraded\":" << BoolJson(status.d435iDegraded) << ","
        << "\"degraded_reason\":" << JsonString(status.d435iDegradedReason) << ","
        << "\"color_valid\":" << BoolJson(status.d435iColorValid) << ","
        << "\"color_fresh\":" << BoolJson(status.d435iColorFresh) << ","
        << "\"depth_valid\":" << BoolJson(status.d435iDepthValid) << ","
        << "\"depth_fresh\":" << BoolJson(status.d435iDepthFresh) << ","
        << "\"color_seq\":" << status.d435iColorSeq << ","
        << "\"depth_seq\":" << status.d435iDepthSeq << ","
        << "\"color_age_s\":" << status.d435iColorAgeSeconds << ","
        << "\"depth_age_s\":" << status.d435iDepthAgeSeconds << ","
        << "\"bag_path\":" << JsonString(status.d435iBagPath)
        << "},"
        << "\"pending_label\":{"
...
struct WebUiServer::Impl
{
    ...
    void HandleGet(int clientFd, const HttpRequest& request)
    {
        ...
        if (path == "/api/d435i/color.jpg")
        {
            const std::vector<uint8_t> jpegBytes = config.d435iColorJpegProvider ? config.d435iColorJpegProvider() : std::vector<uint8_t>{};
            if (jpegBytes.empty())
            {
                SendAll(clientFd, BuildHttpResponse(404, "Not Found", "text/plain; charset=utf-8", "d435i color unavailable"));
                return;
            }
            SendAll(clientFd, BuildBinaryHttpResponse(200, "OK", "image/jpeg", jpegBytes));
            return;
        }
        if (path == "/api/d435i/depth.jpg")
        {
            const std::vector<uint8_t> jpegBytes = config.d435iDepthJpegProvider ? config.d435iDepthJpegProvider() : std::vector<uint8_t>{};
            if (jpegBytes.empty())
            {
                SendAll(clientFd, BuildHttpResponse(404, "Not Found", "text/plain; charset=utf-8", "d435i depth unavailable"));
                return;
            }
            SendAll(clientFd, BuildBinaryHttpResponse(200, "OK", "image/jpeg", jpegBytes));
            return;
        ...
        {
            HandleMjpegStream(clientFd, config.nextImageFrameProvider);
            return;
        }
        if (path == "/api/d435i/color.mjpeg")
        {
            HandleMjpegStream(clientFd, config.nextD435iColorFrameProvider);
            return;
        }
        if (path == "/api/d435i/depth.mjpeg")
        {
            HandleMjpegStream(clientFd, config.nextD435iDepthFrameProvider);
            return;
        }
    ...
    void HandleMjpegStream(
        int clientFd,
        const std::function<UiImageFrame(uint64_t, int)>& nextFrameProvider)
    {
        ...
        while (running.load())
        {
            const UiImageFrame frame = nextFrameProvider(lastSequence, 1000);
            if (!running.load())
            {
                return;
            }
