...
float SlewTowards(float current, float target, float accelPerSecond, float decelPerSecond, float deltaSeconds)
{
    ...
    // Treat direction reversal as braking first so we decelerate to zero
    // before ramping into the opposite direction.
    const bool reversingDirection = std::fabs(target) > 1e-6f && current * target < 0.0f;
    const bool accelerating = !reversingDirection && std::fabs(target) > std::fabs(current);
    const float clampedDeltaSeconds = std::clamp(deltaSeconds, 0.0f, 0.08f);
    const float maxStep = (accelerating ? accelPerSecond : decelPerSecond) * clampedDeltaSeconds;
