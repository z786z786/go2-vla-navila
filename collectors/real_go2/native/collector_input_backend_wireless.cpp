...
constexpr float kTurningLongitudinalScale = 0.35f;
...
struct PendingWirelessControllerActions
{
    ...
    bool Any() const
    {
        return start || select || A || Y || R2 || F1 || F2 || up || right || down || left;
    }

    void MergeFrom(const Gamepad& gamepad)
    {
        start = start || gamepad.start.onPress;
        select = select || gamepad.select.onPress;
...
public:
    ...
    CommandIntent ComputeCommand(const ComputeCommandOptions& options) override
    {
        ...
        const float rawDeltaSeconds = lastCommandUpdate_.time_since_epoch().count() == 0
                                          ? 0.0f
                                          : std::chrono::duration<float>(options.nowSteady - lastCommandUpdate_).count();
        // Clamp large scheduler / DDS gaps so the first recovered step does not jump.
        const float deltaSeconds = std::clamp(rawDeltaSeconds, 0.0f, 0.05f);
        ...
        if (snapshot.valid && ageSeconds >= 0.0 && ageSeconds <= config_.wirelessTimeoutSeconds)
        {
            ...
            if (discreteMotionRequested)
            {
                ...
                // While turning, keep only a low crawl instead of full
                // longitudinal speed through the turn.
                if (std::fabs(targetSendWz) > 1e-6f && std::fabs(targetSendVx) > 1e-6f)
                {
                    targetSendVx *= kTurningLongitudinalScale;
                }
            }
        }
