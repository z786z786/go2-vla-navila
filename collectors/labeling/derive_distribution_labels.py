...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--max-processing-quality-label",
        type=int,
        default=DEFAULT_MAX_PROCESSING_QUALITY_LABEL,
        help="仅为 quality_label 不高于该值且保留帧的 episode 生成标签；默认 2",
    )
    return parser
    ...
    for session_root in session_roots:
        ...
        for episode_meta in index_payload.get("episodes") or []:
            ...
            if quality_review.get("exclude_from_processing"):
                ...
                continue
            if not processing_quality_ok(quality_review.get("quality_label"), max_processing_quality_label):
                skipped_quality_filtered += 1
                continue
...
def main() -> None:
    ...
    summary = generate_distribution_labels(
        ...
        max_processing_quality_label=args.max_processing_quality_label,
    )
    print(
